...
class AdvDiffuser(nn.Module):
    def __init__(self, diffusion_model, num_classes, device):
        ...
        self.device = device
        # gap 把输入的最后两维度变成了 1x1
        self.gap = nn.AdaptiveAvgPool2d(1)
        # model指的是unet里的最后一层卷积输出
        # 本代码中具体实现里是 1（默认为2）
        self.fc = nn.Linear(diffusion_model.model.final_conv.out_channels, num_classes)
        self.target_layer = "model.final_conv"

    # 这个会把每张图片（b c h w ）输入fc层，返回 （B，class_num），图像被分类的概率
    # 那这个函数和下面的classify函数有什么区别？
    # 为什么加个扩散模型？

    # 解答： generate_grad_cam_heatmap这个函数中用到了，因为需要拿到扩散模型的计算中间状态
    def forward(self, x):
    ...
    def pretrain_classify_part(self, epochs, train_loader, device):
        ...
        optimizer = torch.optim.Adam(
            list(self.gap.parameters()) + list(self.fc.parameters()), lr=0.001
        )
    ...
    def generate_adversarial_example(
        self, x, class_idx, timesteps, iterations=2, clipped_reverse_diffusion=True
    ):
        ...
        for batch_idx in range(x.shape[0]):
            ...
            for time_step in range(current_t, -1, -1):
                ...
                current_t_tensor = torch.tensor(
                    [time_step], device=current_x.device, dtype=torch.long
                )

                if clipped_reverse_diffusion:
                    z_t = self.diffusion_model._reverse_diffusion_with_clip(
                        current_x, current_t_tensor, current_noise
                    )
                else:
                    z_t = self.diffusion_model._reverse_diffusion(
                        current_x, current_t_tensor, current_noise
                    )

                if time_step < current_t / 4:# 在扩散过程的后1/4阶段开始进行对抗攻击
                    for _ in range(iterations):
                        # predict
                        # 检查当前图像的预测类别
                        # 如果已经成功欺骗模型（预测类别不等于目标类别），则停止攻击
                        z_t.requires_grad_()
                        ...
                        loss = F.cross_entropy(
                            logits, class_idx[batch_idx].unsqueeze(0)
                        )
                        loss.backward()
                    ...
                    # combine
                    current_x = (
                        x_sample * heatmap[batch_idx] + z_t * inverse_heatmap[batch_idx]
                    )
                else:
                    current_x = z_t
    ...
    def denoise_test_data(self, x, timesteps, clipped_reverse_diffusion=True):
        ...
        for batch_idx in range(x.shape[0]):
            ...
            for time_step in range(current_t, -1, -1):
                ...
                if clipped_reverse_diffusion:
                    current_x = self.diffusion_model._reverse_diffusion_with_clip(
                        current_x, current_t_tensor, current_noise
                    )
                else:
                    current_x = self.diffusion_model._reverse_diffusion(
                        current_x, current_t_tensor, current_noise
                    )

            current_x = (current_x + 1.0) / 2.0
            current_x = torch.clamp(current_x, min=0, max=1).detach()
            x_t[batch_idx] = current_x

...
def main():
    # 导入扩散模型
    model = diffusionNet.DiffusionNet(
        timesteps=timesteps,
        image_size=28,
        in_channels=1,
        base_dim=base_dim,
    ...
    if os.path.exists(ckpt_path):
        ...
        for i in range(epochs):
            ...
            samples = model_ema.module.sampling(
                n_samples, clipped_reverse_diffusion=clip_flag, device=device
            )
            save_image(
                samples,
    ...
    for images_batch, labels_batch in sampled_data_loader:
        ...
        adv_batch = adv_model.generate_adversarial_example(
            images_batch, labels_batch, timesteps
        )
